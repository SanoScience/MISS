...
def mc_sequential_rounding(rho, Z, C_0, mc_C_0, compute_loss_from_scores_real, get_L0_penalty, mc_get_L0_penalty, objval_cutoff=float('Inf')):
    """
    ...
    X, Y = Z
    rho_reshaped = rho.reshape((X.shape[-1], Y.shape[-1]))
    scores = X.dot(rho_reshaped)
    best_objval = compute_loss_from_scores_real(scores) + get_L0_penalty(rho) + mc_get_L0_penalty(rho)

    while len(dimensions_to_round) > 0 and best_objval < objval_cutoff:

        objvals_at_floor = np.repeat(np.nan, P)
        objvals_at_ceil = np.repeat(np.nan, P)
        current_penalty = get_L0_penalty(rho) + mc_get_L0_penalty(rho)

        base_scores = np.copy(scores)
        for idx in dimensions_to_round:
            feature_idx = idx // Y.shape[-1]
            class_idx = idx % Y.shape[-1]
            # scores go from center to ceil -> center + dist_from_start_to_ceil
            X_dim = X[:, feature_idx]
            base_scores[:, class_idx] = scores[:, class_idx] + dist_from_start_to_ceil[idx] * X_dim
            objvals_at_ceil[idx] = compute_loss_from_scores_real(base_scores)

            # move from ceil to floor => -1*Z_j
            base_scores[:, class_idx] = base_scores[:, class_idx] - X_dim
            objvals_at_floor[idx] = compute_loss_from_scores_real(base_scores)
        ...
        if best_objval_at_ceil <= best_objval_at_floor:
            best_objval = best_objval_at_ceil
            best_dim = np.nanargmin(objvals_at_ceil)
            best_feature_idx = best_dim // Y.shape[-1]
            best_class_idx = best_dim % Y.shape[-1]
            rho[best_dim] += dist_from_start_to_ceil[best_dim]
            scores[:, best_class_idx] = scores[:, best_class_idx] + dist_from_start_to_ceil[best_dim] * X[:, best_feature_idx]
        else:
            best_objval = best_objval_at_floor
            best_dim = np.nanargmin(objvals_at_floor)
            best_feature_idx = best_dim // Y.shape[-1]
...
def mc_discrete_descent(rho, Z, C_0, mc_C_0, rho_ub, rho_lb, get_L0_penalty, mc_get_L0_penalty, compute_loss,
                        compute_loss_from_scores,
    ...
    X, Y = Z
    rho_reshaped = rho.reshape((X.shape[-1], Y.shape[-1]))
    base_scores = X.dot(rho_reshaped)
    base_loss = compute_loss(rho)
    base_objval = base_loss + get_L0_penalty(rho) + mc_get_L0_penalty(rho)
    n_iterations = 0
    ...
    while n_iterations < MAX_ITERATIONS and len(search_dimensions) > 0:
        ...
        for k in search_dimensions:
            dim_objvals = _mc_compute_objvals_at_dim(base_rho=rho,
                                                     base_scores=base_scores,
                                                     base_loss=base_loss,
                                                     dim_idx=k,
                                                     dim_coefs=coefficient_values[k],
        ...
        rho[best_idx] += best_step
        base_objval = next_objval
        base_loss = base_objval - get_L0_penalty(rho) - mc_get_L0_penalty(rho)
        base_scores[:, best_idx % Y.shape[-1]] = base_scores[:, best_idx % Y.shape[-1]] + (
                    best_step * X[:, best_idx // Y.shape[-1]])
...
def _mc_compute_objvals_at_dim(Z, C_0, mc_C_0, base_rho, base_scores, base_loss, dim_coefs, dim_idx, compute_loss,
                               compute_loss_from_scores):
    """
    finds the value of rho[j] in dim_coefs that minimizes log_loss(rho) + C_0j
    ...
    scores = np.copy(base_scores)
    X, Y = Z
    rho_reshaped = base_rho.reshape((X.shape[-1], Y.shape[-1]))
    feature_idx = dim_idx // Y.shape[-1]
    class_idx = dim_idx % Y.shape[-1]
    ...
    for i in range(n_forward_steps):
        scores[:, class_idx] = scores[:, class_idx] + (forward_step_sizes[i] * X_dim)
        total_distance_from_base += forward_step_sizes[i]
        current_loss = compute_loss_from_scores(scores)
        if current_loss >= best_loss:
            stop_after_first_forward_step = i == 0
    ...
    if move_backward:
        ...
        for i in range(n_backward_steps):
            scores[:, class_idx] = scores[:, class_idx] + (backward_step_sizes[i] * X_dim)
            total_distance_from_base += backward_step_sizes[i]
            current_loss = compute_loss_from_scores(scores)
            if current_loss >= best_loss:
                break
    ...
    nonzero_features = (np.count_nonzero(base_rho.reshape(X.shape[-1], -1), axis=1) > 0)
    nonzero_features[feature_idx] = False

    other_features_penalty = np.sum(mc_C_0[nonzero_features])
    objval_at_coef_values = loss_at_coef_value + other_dim_penalty + other_features_penalty

    if C_0[dim_idx] > 0.0 or mc_C_0[feature_idx] > 0.0:

        # increase objective value at every non-zero coefficient value by C_0j
        nonzero_coef_idx = np.flatnonzero(dim_coefs)
        objval_at_coef_values[nonzero_coef_idx] = objval_at_coef_values[nonzero_coef_idx] + C_0[dim_idx] + mc_C_0[feature_idx]
        ...
        if np.isnan(objval_at_coef_values[zero_coef_idx]):
            ...
            # steps_from_here_to_zero: -step_from_base_to_here + -step_from_zero_to_base
            steps_to_zero = -(base_coef_value + total_distance_from_base)
            scores[:, class_idx] = scores[:, class_idx] + steps_to_zero * X_dim
            objval_at_coef_values[zero_coef_idx] = compute_loss_from_scores(scores) + other_dim_penalty + other_features_penalty
            # assert(all(np.isclose(scores, base_scores - base_coef_value * Z_dim)))
