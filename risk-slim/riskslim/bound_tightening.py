...
def mc_chained_updates(bounds, mc_C_0_nnz, new_objval_at_feasible=None, new_objval_at_relaxation=None, MAX_CHAIN_COUNT=20):
    new_bounds = dict(bounds)
    ...
    while improved_bounds and chain_count < MAX_CHAIN_COUNT:
        ...
        improved_bounds = False
        L0_penalty_min = np.sum(np.sort(mc_C_0_nnz)[np.arange(int(new_bounds['mc_L0_min']))])
        L0_penalty_max = np.sum(-np.sort(-mc_C_0_nnz)[np.arange(int(new_bounds['mc_L0_max']))])
        ...
        # L0_min
        if new_bounds['objval_min'] > new_bounds['loss_max']:
            proposed_L0_min = np.ceil((new_bounds['objval_min'] - new_bounds['loss_max']) / np.min(mc_C_0_nnz))
            if proposed_L0_min > new_bounds['mc_L0_min']:
                new_bounds['mc_L0_min'] = proposed_L0_min
                improved_bounds = True
        ...
        # L0_max
        if new_bounds['objval_max'] > new_bounds['loss_min']:
            proposed_L0_max = np.floor((new_bounds['objval_max'] - new_bounds['loss_min']) / np.min(mc_C_0_nnz))
            if proposed_L0_max < new_bounds['mc_L0_max']:
                new_bounds['mc_L0_max'] = proposed_L0_max
                improved_bounds = True
...
def chained_updates_for_lp(bounds, C_0_nnz, new_objval_at_feasible=None, new_objval_at_relaxation=None,
                           MAX_CHAIN_COUNT=20):
    new_bounds = dict(bounds)
