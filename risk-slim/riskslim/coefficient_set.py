...
class CoefficientSet(object):
    ...
    def __init__(self, variable_names, omit_init=False, **kwargs):
        if omit_init:
            return
        ...
        for name in variable_names:
            ...
            self._coef_elements[name] = _CoefficientElement(name=name, ub=ub[idx], lb=lb[idx], c0=c0[idx],
                                                            vtype=vtype[idx])
    ...
    def _check_rep(self):
        ...
        if self._correct_flag:
            ...
            for name in self._variable_names:
                ...
                if name in {'Intercept', '(Intercept)', 'intercept', '(intercept)'} or 'Intercept' in name:
...
class MCCoefficientSet(CoefficientSet):
    """
    Class used to represent and manipulate constraints on individual coefficients
    including upper bound, lower bound, variable type, and regularization in the Multiclass setting.
    Coefficient Set is composed of Coefficient Elements
    """

    def __init__(self, variable_names, num_classes: int, **kwargs):
        # set variables using setter methods
        super().__init__(variable_names, omit_init=True, **kwargs)
        variable_names = list(variable_names)
        variable_names = [f"{name}_cl{i}" for name in variable_names for i in range(num_classes)]
        ...
        lb = kwargs.get('lb', _CoefficientElement._DEFAULT_LB)
        c0 = kwargs.get('c0', _CoefficientElement._DEFAULT_c0)
        max_intercept = kwargs.get('max_intercept', _CoefficientElement._DEFAULT_UB)
        vtype = kwargs.get('type', _CoefficientElement._DEFAULT_TYPE)
        ...
        for name in variable_names:
            ...
            if "Intercept" in name:
                self._coef_elements[name] = _CoefficientElement(name=name, ub=max_intercept, lb=-max_intercept,
                                                                c0=c0[idx], vtype=vtype[idx])
            else:
                self._coef_elements[name] = _CoefficientElement(name=name, ub=ub[idx], lb=lb[idx], c0=c0[idx],
                                                                vtype=vtype[idx])
